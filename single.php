...
while ( have_posts() ) :
	...
	<article <?php post_class(); ?>>
		...
		<!-- Hero -->
		<header class="relative overflow-hidden bg-night-wash text-paper-100">
			<div class="absolute inset-0 bg-grain opacity-25" aria-hidden="true"></div>
			<?php if ( has_post_thumbnail() ) : ?>
				<div class="absolute inset-0">
					<?php the_post_thumbnail( 'nora-hero', array( 'class' => 'h-full w-full object-cover opacity-25' ) ); ?>
					<div class="absolute inset-0 bg-night-wash/80"></div>
				</div>
			<?php endif; ?>
			<div class="container-nora relative flex flex-col items-center gap-5 py-20 text-center">
				...
				<div class="flex flex-wrap items-center justify-center gap-x-4 gap-y-2 text-sm text-paper-300">
					...
					<span class="text-paper-500">·</span>
					<span><?php echo esc_html( nora_learn_reading_time() ); ?></span>
				</div>
			</div>
		</header>
		...
		<div class="section-tight">
			<div class="container-nora">
				<div class="mx-auto max-w-3xl">
					...
					wp_link_pages(
						array(
							'before' => '<div class="mt-8 flex items-center gap-2 text-sm font-semibold">' . esc_html__( 'หน้า:', 'nora-learn' ),
							'after'  => '</div>',
						)
					);
					?>
					...
					<?php if ( has_tag() ) : ?>
						<div class="mt-10 flex flex-wrap items-center gap-2 border-t border-paper-200 pt-8">
							<span class="text-sm font-semibold text-ink-soft"><?php esc_html_e( 'แท็ก:', 'nora-learn' ); ?></span>
							<?php
							foreach ( get_the_tags() as $tag ) {
								printf( '<a href="%s" class="badge-muted hover:bg-gold-50 hover:text-gold">#%s</a>', esc_url( get_tag_link( $tag ) ), esc_html( $tag->name ) );
							}
							?>
						</div>
					<?php endif; ?>
					...
					<?php if ( get_the_author_meta( 'description' ) ) : ?>
						<div class="mt-10 flex gap-5 rounded-2xl border border-paper-200 bg-paper-100/60 p-6">
							...
							<div>
								<p class="text-xs uppercase tracking-wider text-gold"><?php esc_html_e( 'เขียนโดย', 'nora-learn' ); ?></p>
		...
		if ( $related->have_posts() ) :
			...
			<section class="section bg-paper-100/50">
				<div class="container-nora">
					<?php nora_learn_section_heading( array( 'eyebrow' => __( 'อ่านเพิ่มเติม', 'nora-learn' ), 'title' => __( 'บทความที่เกี่ยวข้อง', 'nora-learn' ), 'align' => 'left' ) ); ?>
