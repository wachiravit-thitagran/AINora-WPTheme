...
flowchart TD
    ...
    WP --> Theme["Nora Learn classic theme"]
...
| Path | หน้าที่ |
| --- | --- |
| `functions.php` | Bootstrap ธีม, define `NORA_LEARN_VERSION`, `NORA_LEARN_DIR`, `NORA_LEARN_URI`, โหลดไฟล์ใน `inc/`, และโหลด `inc/tutor.php` เฉพาะเมื่อ Tutor LMS active |
...
flowchart TD
    ...
    TutorHelpers --> Require["nora_learn_require()"]
    ...
    TutorCheck{"nora_learn_has_tutor_lms()"} -->|yes| Tutor["inc/tutor.php"]
...
sequenceDiagram
    ...
    participant Theme as Nora Learn Theme
...
flowchart LR
    ...
    Customizer["Customizer theme_mod"] --> ThemeOptions["nora_learn_option()"]
    ...
    Courses --> Stats["nora_learn_get_stats()"]
    Users --> Stats
    Stats --> Transient["Transient: nora_learn_stats"]
...
- Transient cache: `nora_learn_stats` cache สถิติ 1 ชั่วโมง และถูกล้างเมื่อ course/enrolment/completion เปลี่ยน
- Filters สำหรับปรับแต่งโดยไม่แก้ template: `nora_learn_content_width`, `nora_learn_faq_items`, `nora_learn_partner_logos`, `nora_learn_featured_meta_key`, รวมถึง Tutor filters ที่ธีม hook ไว้
...
flowchart TD
    ...
    Build --> Package["Package dist/nora-learn.zip"]
    ...
    WPUpdate --> Site["Installed Nora Learn sites"]
```

`inc/updater.php` ใช้ Plugin Update Checker ที่ vendored ไว้ใน `inc/lib/plugin-update-checker/` และเลือก release asset ที่ชื่อ `nora-learn.zip` ก่อน source zip เพื่อให้ folder name และ compiled assets ถูกต้อง Release workflow จะไม่ทำงานเมื่อ push แตะเฉพาะ markdown, `docs/**`, หรือ `.github/**`
...
- Guard Tutor API usage with `nora_learn_has_tutor_lms()`, `nora_learn_tutor_utils()`, or `method_exists()`
