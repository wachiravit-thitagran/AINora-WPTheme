...
<article <?php post_class( 'card card-hover group flex flex-col' ); ?>>
	<a href="<?php the_permalink(); ?>" class="relative block aspect-[16/10] overflow-hidden bg-paper-100">
		...
		<?php else : ?>
			<span class="flex h-full w-full items-center justify-center bg-gold-wash text-paper-50">
				<?php echo nora_learn_icon( 'lotus', 'h-12 w-12 opacity-60' ); // phpcs:ignore ?>
			</span>
		<?php endif; ?>
		<?php if ( ! empty( $cat ) ) : ?>
	...
	<div class="flex flex-1 flex-col gap-3 p-6">
		<div class="flex items-center gap-2 text-xs text-ink-light">
			<?php echo nora_learn_icon( 'calendar', 'h-3.5 w-3.5 text-gold' ); // phpcs:ignore ?>
		...
		<a href="<?php the_permalink(); ?>" class="mt-auto inline-flex items-center gap-1.5 pt-2 text-sm font-semibold text-gold">
			<?php esc_html_e( 'อ่านต่อ', 'nora-learn' ); ?>
