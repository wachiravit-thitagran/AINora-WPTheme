...
<article class="card card-hover group flex flex-col items-center p-8 text-center">
	<a href="<?php echo esc_url( $profile ); ?>" class="relative">
		<span class="absolute -inset-1 rounded-full bg-gold/10 opacity-0 transition group-hover:opacity-100" aria-hidden="true"></span>
	...
	<h3 class="mt-5 font-serif text-lg font-bold text-ink transition group-hover:text-gold">
	...
	<?php if ( $courses ) : ?>
		<div class="mt-4 inline-flex items-center gap-1.5 text-xs font-medium text-gold">
			<?php echo nora_learn_icon( 'book', 'h-4 w-4' ); // phpcs:ignore ?>
			<?php printf( esc_html__( '%d คอร์ส', 'nora-learn' ), $courses ); ?>
		</div>
	<?php endif; ?>
</article>
