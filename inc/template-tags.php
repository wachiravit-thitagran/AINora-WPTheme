 ...
 */
function nora_learn_breadcrumb() {
	if ( is_front_page() ) {
		return;
	}

	$home_label = __( 'หน้าแรก', 'nora-learn' );
	$items      = array();
	$items[]    = '<a href="' . esc_url( home_url( '/' ) ) . '" class="hover:text-gold">' . esc_html( $home_label ) . '</a>';

	if ( is_singular( 'post' ) ) {
		$blog_id = (int) get_option( 'page_for_posts' );
		if ( $blog_id ) {
			$items[] = '<a href="' . esc_url( get_permalink( $blog_id ) ) . '" class="hover:text-gold">' . esc_html( get_the_title( $blog_id ) ) . '</a>';
		}
		$items[] = '<span class="text-ink-soft">' . esc_html( wp_trim_words( get_the_title(), 8, '…' ) ) . '</span>';
	} elseif ( is_page() ) {
		$ancestors = array_reverse( get_post_ancestors( get_the_ID() ) );
		foreach ( $ancestors as $ancestor ) {
			$items[] = '<a href="' . esc_url( get_permalink( $ancestor ) ) . '" class="hover:text-gold">' . esc_html( get_the_title( $ancestor ) ) . '</a>';
		}
		...
		$items[] = '<span class="text-ink-soft">' . sprintf( /* translators: %s: search term */ esc_html__( 'ผลการค้นหา: %s', 'nora-learn' ), esc_html( get_search_query() ) ) . '</span>';
	} elseif ( is_archive() ) {
		$items[] = '<span class="text-ink-soft">' . esc_html( wp_strip_all_tags( get_the_archive_title() ) ) . '</span>';
	} elseif ( is_404() ) {
		$items[] = '<span class="text-ink-soft">' . esc_html__( 'ไม่พบหน้า', 'nora-learn' ) . '</span>';
	}
	...
	echo '<nav class="container-nora flex items-center gap-2 py-4 text-sm text-ink-light" aria-label="' . esc_attr__( 'เส้นทางนำทาง', 'nora-learn' ) . '">';
 ...
 */
function nora_learn_pagination() {
	$links = paginate_links(
		array(
			'type'      => 'array',
			'mid_size'  => 1,
			'prev_text' => '<span aria-hidden="true">&larr;</span><span class="sr-only">' . esc_html__( 'ก่อนหน้า', 'nora-learn' ) . '</span>',
			'next_text' => '<span class="sr-only">' . esc_html__( 'ถัดไป', 'nora-learn' ) . '</span><span aria-hidden="true">&rarr;</span>',
		)
	);
	...
	echo '<nav class="mt-12 flex justify-center" aria-label="' . esc_attr__( 'แบ่งหน้า', 'nora-learn' ) . '"><ul class="flex flex-wrap items-center gap-2">';
	foreach ( $links as $link ) {
		$is_current = strpos( $link, 'current' ) !== false;
		$classes    = $is_current
			? 'flex h-10 min-w-10 items-center justify-center rounded-full bg-gold px-3 font-semibold text-white'
			: 'flex h-10 min-w-10 items-center justify-center rounded-full border border-paper-200 bg-white px-3 text-ink-soft transition hover:border-gold hover:text-gold';
...
function nora_learn_post_meta() {
	...
	printf(
		...
		nora_learn_icon( 'calendar', 'h-4 w-4 text-gold' ),
		esc_html( get_the_date() )
	);
	echo '<span class="text-paper-400">·</span>';
	printf(
		'<span class="inline-flex items-center gap-1.5">%1$s%2$s</span>',
		nora_learn_icon( 'clock', 'h-4 w-4 text-gold' ),
		esc_html( nora_learn_reading_time() )
	);
	if ( $cats ) {
		echo '<span class="text-paper-400">·</span>';
		echo '<span class="inline-flex flex-wrap items-center gap-1 [&_a]:text-gold [&_a:hover]:underline">' . wp_kses_post( $cats ) . '</span>';
	}
	echo '</div>';
}
 ...
 */
function nora_learn_reading_time() {
	...
	return sprintf( _n( 'อ่าน %d นาที', 'อ่าน %d นาที', $minutes, 'nora-learn' ), $minutes );
}
 ...
 */
function nora_learn_icon( $name, $classes = 'h-5 w-5' ) {
 ...
 */
function nora_learn_section_heading( $args = array() ) {
	$args = wp_parse_args(
		$args,
		array(
			'eyebrow' => '',
 ...
 */
function nora_learn_page_hero( $args = array() ) {
	$args = wp_parse_args(
		$args,
		array(
			'title'      => get_the_title(),
	...
	?>
	<section class="relative overflow-hidden bg-night-wash text-paper-100">
		<div class="absolute inset-0 bg-grain opacity-30" aria-hidden="true"></div>
		<div class="pointer-events-none absolute -right-16 -top-20 h-72 w-72 rounded-full bg-gold/30 blur-3xl" aria-hidden="true"></div>
		<div class="container-nora relative flex flex-col gap-4 py-16 sm:py-20 <?php echo esc_attr( $align ); ?>">
			<?php if ( $args['eyebrow'] ) : ?>
 ...
 * @return mixed
 */
function nora_learn_option( $key, $default = '' ) {
 ...
 */
function nora_learn_get_stats() {
	$cached = get_transient( 'nora_learn_stats' );
	if ( is_array( $cached ) ) {
		return $cached;
	}
	...
	set_transient( 'nora_learn_stats', $stats, HOUR_IN_SECONDS );
	return $stats;
}
 ...
 * @return WP_User[]
 */
function nora_learn_get_instructors( $limit = -1 ) {
	$tutils = nora_learn_tutor_utils();
 ...
 * @return WP_User|null
 */
function nora_learn_course_instructor( $course_id ) {
	$tutils = nora_learn_tutor_utils();
	$user   = null;
 ...
 */
function nora_learn_course_progress( $course_id, $user_id = 0 ) {
	$tutils = nora_learn_tutor_utils();
	if ( ! $tutils ) {
		return null;
	}
 ...
 * `nora_learn_faq_items` filter. Each item: array( 'q' => ..., 'a' => ... ).
 *
 * @return array<int, array{q:string,a:string}>
 */
function nora_learn_get_faqs() {
	$defaults = array(
		array(
			'q' => __( 'การสมัครเรียนมีค่าใช้จ่ายหรือไม่?', 'nora-learn' ),
			'a' => __( 'คอร์สส่วนใหญ่บนแพลตฟอร์มเปิดให้เรียนฟรี เพียงสมัครสมาชิกก็เริ่มเรียนได้ทันที บางคอร์สอาจมีค่าใช้จ่ายซึ่งจะระบุไว้อย่างชัดเจนในหน้าคอร์ส', 'nora-learn' ),
		),
		array(
			'q' => __( 'ต้องเรียนตามเวลาที่กำหนดไหม?', 'nora-learn' ),
			'a' => __( 'ไม่จำเป็น คุณสามารถเรียนได้ทุกที่ทุกเวลาตามจังหวะของตัวเอง ระบบจะบันทึกความคืบหน้าให้อัตโนมัติ', 'nora-learn' ),
		),
		array(
			'q' => __( 'เรียนจบแล้วได้รับเกียรติบัตรหรือไม่?', 'nora-learn' ),
			'a' => __( 'คอร์สที่เปิดให้มีเกียรติบัตร เมื่อคุณเรียนและทำแบบทดสอบครบตามเงื่อนไข ระบบจะออกเกียรติบัตรให้ดาวน์โหลดได้จากแดชบอร์ด “เกียรติบัตรของฉัน”', 'nora-learn' ),
		),
		array(
			'q' => __( 'ลืมรหัสผ่านต้องทำอย่างไร?', 'nora-learn' ),
			'a' => __( 'คลิก “เข้าสู่ระบบ” แล้วเลือก “ลืมรหัสผ่าน” กรอกอีเมลที่ใช้สมัคร ระบบจะส่งลิงก์สำหรับตั้งรหัสผ่านใหม่ให้ทางอีเมล', 'nora-learn' ),
		),
		array(
			'q' => __( 'ดูประวัติการเรียนได้ที่ไหน?', 'nora-learn' ),
			'a' => __( 'เข้าสู่ระบบแล้วไปที่แดชบอร์ดของฉัน จะเห็นคอร์สที่ลงทะเบียน ความคืบหน้า และเกียรติบัตรทั้งหมด', 'nora-learn' ),
		),
	);

	return (array) apply_filters( 'nora_learn_faq_items', $defaults );
}
