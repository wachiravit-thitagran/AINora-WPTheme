 ...
 * @return bool
 */
function nora_learn_seo_plugin_active() {
 ...
 * @return array
 */
function nora_learn_schema_organization() {
	$org = array(
		'@type' => 'Organization',
	...
	$socials = array_filter(
		array(
			nora_learn_option( 'nora_social_facebook' ),
			nora_learn_option( 'nora_social_youtube' ),
			nora_learn_option( 'nora_social_line' ),
		)
	);
	if ( $socials ) {
 ...
 * breadcrumb in nora_learn_breadcrumb()). Returns null on the front page.
 *
 * @return array|null
 */
function nora_learn_schema_breadcrumb() {
	if ( is_front_page() ) {
		return null;
	}
	...
	if ( is_singular( 'post' ) ) {
		...
		$items[] = array(
			'name' => sprintf( __( 'ผลการค้นหา: %s', 'nora-learn' ), get_search_query() ),
			'url'  => '',
		);
	} else {
		return null;
 ...
 * @return array|null
 */
function nora_learn_schema_article() {
	$post_id = get_the_ID();
	if ( ! $post_id ) {
		return null;
	}
	...
	if ( has_post_thumbnail() ) {
		$src = wp_get_attachment_image_src( get_post_thumbnail_id(), 'nora-hero' );
		if ( $src ) {
			$article['image'] = array(
				'@type'  => 'ImageObject',
				'url'    => $src[0],
 ...
 * @return array|null
 */
function nora_learn_schema_faq() {
	if ( ! function_exists( 'nora_learn_get_faqs' ) ) {
		return null;
	}
 ...
 */
function nora_learn_print_schema() {
	if ( nora_learn_seo_plugin_active() ) {
		return;
	}

	$graph = array( nora_learn_schema_organization() );

	$breadcrumb = nora_learn_schema_breadcrumb();
	if ( $breadcrumb ) {
		$graph[] = $breadcrumb;
	}

	if ( is_singular( 'post' ) ) {
		$article = nora_learn_schema_article();
		if ( $article ) {
			$graph[] = $article;
		}
	}
...
add_action( 'wp_head', 'nora_learn_print_schema', 20 );
